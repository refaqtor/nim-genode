...
proc construct(conn: Connection; env: GenodeEnv; label: cstring) {.tags: [RpcEffect],
  importcpp: "#.construct(*#, @, false, false)".}
...
proc newAudioOutClient*(env: GenodeEnv): AudioOutClient =
  new result
  result.left.construct(env, "left")
  result.right.construct(env, "right")
